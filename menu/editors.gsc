...
{        
    ...
    while( curs <= max-1 )
    {
        ...
        progress[progress.size] = self createText("objective", .7, "RIGHT", "CENTER", xPosition + 108, progress[progress.size-2].y, 5, 1, int( min(curs, 100) ), (1,1,1));
        progress[progress.size] = self createText("objective", .7, "RIGHT", "CENTER", xPosition + 126, progress[progress.size-2].y, 5, 1, "/" + max, (1,1,1));
...
areYouSure()
{
    ...
    youSure[youSure.size] = self createText("small", .7, "LEFT", "CENTER", xPos + 183, self.menu["OPT"][cap_curs].y, 6, 1, " Yes   No", (1,1,1));
    youSure[youSure.size] = self createText("small", .7, "RIGHT", "CENTER", xPos + 180, self.menu["OPT"][cap_curs].y, 5, 1, "Are You Sure?", (1,1,1));
    wait .2;
